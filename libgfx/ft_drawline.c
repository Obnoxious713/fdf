...
void				init_color_tab(t_data *data)
{
	...
	while (++i < 200)
	{
		r[2] = ((float)(r[0] * i) / 200) + ((float)(r[1] * (200 - i)) / 200);
		g[2] = ((float)(g[0] * i) / 200) + ((float)(g[1] * (200 - i)) / 200);
		b[2] = ((float)(b[0] * i) / 200) + ((float)(b[1] * (200 - i)) / 200);
...
static void			draw_point(t_data *data, int x, int y, float z)
{
	...
	if (x > 0 && y > 0 && x < WINDOW_SIZE_X && y < WINDOW_SIZE_Y)
	{
		...
		{	cz = ((z - data->plot->z_min)
			/ (data->plot->z_max - data->plot->z_min)) * 200;
		}
