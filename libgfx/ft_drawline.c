...
void				init_color_tab(t_data *data)
{
	...
	data->colors = (int*)ft_memalloc(sizeof(int) * 100);
	r[0] = (float)(PURPLE >> 16 & 0xFF);
	r[1] = (float)(AMETHYST >> 16 & 0xFF);
	g[0] = (float)(PURPLE >> 8 & 0xFF);
	g[1] = (float)(AMETHYST >> 8 & 0xFF);
	b[0] = (float)(PURPLE & 0xFF);
	b[1] = (float)(AMETHYST & 0xFF);
	i = -1;
	while (++i < 100)
	{
...
static void			draw_point(t_data *data, int x, int y, float z)
{
	...
	{
		if (data->plot->z_max + data->plot->z_min != 0)
			cz = ((z - data->plot->z_min)
				/ (data->plot->z_max - data->plot->z_min)) * 100;
		color = data->colors[abs((int)cz - 1)];
		i = (x * 4) + (y * data->s_line);
...
void				ft_3d_drawline(t_data *data, t_3d p0, t_3d p1)
{
	...
	{
		draw_point(data, dir ? (int)p0.y : (int)p0.x,
					dir ? (int)p0.x : (int)p0.y, p0.z);
		err += delta_err;
		if (err >= 0.0)
		{
			p0.y += (p0.y > p1.y) ? -1 : 1;
